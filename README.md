...
relationship graph and filterable timeline (Phase 5). The site is a static Astro
build with client-side Pagefind search — no server or database to run.

```bash
npm install
npm run dev        # local dev server at http://localhost:4321
                   #   → browse /, edit at /keystatic, explore /graph and /timeline
...
- **The relationship graph** at `/graph` and the **timeline** at `/timeline` are
  derived at build time from the same frontmatter references that power each
  entity's "Connections" panel; entity pages themselves ship zero JavaScript.
